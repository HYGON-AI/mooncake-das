...
class MooncakeBackend final : public ::c10d::Backend {
   public:
    ...
    std::string getPreferredHca(std::string location) {
        LOG(INFO) << "[PG] Resolving preferred HCA for " << location;
        ...
        const auto& preferred_hca = it->second.preferred_hca[0];
        LOG(INFO) << "[PG] Preferred HCA for " << location << " is "
                  << preferred_hca;
        return preferred_hca;
    }
