...
MooncakeBackend::MooncakeBackend(
    ...
    if (isCpu) {
        ...
        for (size_t i = 0; i < 2; i++) {
            hipError_t err = hipExtMallocWithFlags(
                &send_buffer_[i], kBufferSize, hipDeviceMallocFinegrained);
        ...
        for (size_t i = 0; i < 2; i++) {
            hipError_t err = hipExtMallocWithFlags(
                &recv_buffer_[i], kBufferSize, hipDeviceMallocFinegrained);
    ...
    int cuda_device_index = isCpu_ ? -1 : at::hip::current_device();
    ...
    if (isCpu) {
        ...
        hipHostMalloc(&meta_->activeRanks, kMaxNumRanks * sizeof(bool),
                      hipHostMallocMapped);
...
c10::intrusive_ptr<c10d::Work> MooncakeBackend::send(
    ...
    if (!isCpu_) {
        auto current_stream =
            at::hip::getCurrentHIPStreamMasqueradingAsCUDA(contiguous.device().index());
...
c10::intrusive_ptr<c10d::Work> MooncakeBackend::recv(
    ...
    if (!isCpu_) {
        auto current_stream =
            at::hip::getCurrentHIPStreamMasqueradingAsCUDA(target.device().index());
        stream = current_stream.stream();
    }
...
c10::intrusive_ptr<c10d::Work> MooncakeBackend::broadcast(
    ...
    if (isCpu_) {
        ...
        at::hip::HIPStreamMasqueradingAsCUDA stream =
            at::hip::getCurrentHIPStreamMasqueradingAsCUDA(tensor.device().index());
        return worker_->putTaskCuda(
            c10d::OpType::BROADCAST, tensorSize, root, meta_, connection_ctx_,
            stream,
            [=](void* dst, size_t pos, size_t realSize,
                const at::hip::HIPStreamMasqueradingAsCUDA& enq_stream) {
                if (isRoot) {
                    hipMemcpyAsync(dst, (char*)tensor.data_ptr() + pos,
                                    realSize, hipMemcpyDeviceToDevice,
                                    enq_stream);
                }
            },
            [=](void* src, size_t pos, size_t realSize,
                const at::hip::HIPStreamMasqueradingAsCUDA& enq_stream) {
                hipMemcpyAsync((char*)tensor.data_ptr() + pos, src, realSize,
                                hipMemcpyDeviceToDevice, enq_stream);
            });
    }
}
...
c10::intrusive_ptr<c10d::Work> MooncakeBackend::allreduce(
    ...
    if (isCpu_) {
        ...
        auto stream = at::hip::getCurrentHIPStreamMasqueradingAsCUDA(tensor.device().index());
        return worker_->putTaskCuda(
            c10d::OpType::ALLREDUCE, tensorSize, 0, meta_, connection_ctx_,
            stream,
            [=](void* dst, size_t pos, size_t realSize,
                const at::hip::HIPStreamMasqueradingAsCUDA& enq_stream) {
                hipMemcpyAsync(dst, (char*)tensor.data_ptr() + pos, realSize,
                                hipMemcpyDeviceToDevice, enq_stream);
            },
            [=, this](void* src, size_t pos, size_t realSize,
                      const at::hip::HIPStreamMasqueradingAsCUDA& enq_stream) {
                hipMemsetAsync((char*)tensor.data_ptr() + pos, 0, realSize,
                                enq_stream);
...
c10::intrusive_ptr<c10d::Work> MooncakeBackend::allgather(
    ...
    if (isCpu_) {
        ...
        auto stream =
            at::hip::getCurrentHIPStreamMasqueradingAsCUDA(inputTensor.device().index());
        return worker_->putTaskCuda(
            c10d::OpType::ALLGATHER, tensorSize, 0, meta_, connection_ctx_,
            stream,
            [=](void* dst, size_t pos, size_t realSize,
                const at::hip::HIPStreamMasqueradingAsCUDA& enq_stream) {
                hipMemcpyAsync(dst, (char*)inputTensor.data_ptr() + pos,
                                realSize, hipMemcpyDeviceToDevice, enq_stream);
            },
            [=](void* src, size_t pos, size_t realSize,
                const at::hip::HIPStreamMasqueradingAsCUDA& enq_stream) {
                for (const auto j : c10::irange(outputTensors_.size())) {
                    hipMemcpyAsync((char*)outputTensors_[j].data_ptr() + pos,
                                    (char*)src + j * realSize, realSize,
                                    hipMemcpyDeviceToDevice, enq_stream);
                }
            });
...
c10::intrusive_ptr<c10d::Work> MooncakeBackend::_allgather_base(
    ...
    if (isCpu_) {
        ...
        auto stream =
            at::hip::getCurrentHIPStreamMasqueradingAsCUDA(inputBuffer.device().index());
        return worker_->putTaskCuda(
            c10d::OpType::_ALLGATHER_BASE, tensorSize, 0, meta_,
            connection_ctx_, stream,
            [=](void* dst, size_t pos, size_t realSize,
                const at::hip::HIPStreamMasqueradingAsCUDA& enq_stream) {
                hipMemcpyAsync(dst, (char*)inputBuffer.data_ptr() + pos,
                                realSize, hipMemcpyDeviceToDevice, enq_stream);
            },
            [=, this](void* src, size_t pos, size_t realSize,
                      const at::hip::HIPStreamMasqueradingAsCUDA& enq_stream) {
...
c10::intrusive_ptr<c10d::Work> MooncakeBackend::_reduce_scatter_base(
    ...
    if (isCpu_) {
        ...
        return worker_->putTaskCuda(
            ...
            [=, this](void* src, size_t pos, size_t realSize,
                      const at::hip::HIPStreamMasqueradingAsCUDA& enq_stream) {
                hipMemsetAsync((char*)outputBuffer.data_ptr() + pos, 0,
                                realSize, enq_stream);
...
c10::intrusive_ptr<c10d::Work> MooncakeBackend::alltoall(
    ...
    if (isCpu_) {
        ...
        auto stream =
            at::hip::getCurrentHIPStreamMasqueradingAsCUDA(inputTensors[0].device().index());
        return worker_->putTaskCuda(
            c10d::OpType::ALLTOALL, tensorSize, 0, meta_, connection_ctx_,
            stream,
            [=](void* dst, size_t pos, size_t realSize,
                const at::hip::HIPStreamMasqueradingAsCUDA& enq_stream) {
                for (const auto j : c10::irange(inputTensors.size())) {
                    hipMemcpyAsync((char*)dst + j * realSize,
            ...
            [=](void* src, size_t pos, size_t realSize,
                const at::hip::HIPStreamMasqueradingAsCUDA& enq_stream) {
                for (const auto j : c10::irange(outputTensors.size())) {
                    hipMemcpyAsync((char*)outputTensors[j].data_ptr() + pos,
                                    (char*)src + j * realSize, realSize,
                                    hipMemcpyDeviceToDevice, enq_stream);
                }
            });
...
c10::intrusive_ptr<c10d::Work> MooncakeBackend::barrier(
    ...
    if (isCpu_) {
        ...
        auto device_index = at::hip::current_device();
        auto stream = at::hip::getCurrentHIPStreamMasqueradingAsCUDA(device_index);
        return worker_->putTaskCuda(
            c10d::OpType::BARRIER, kBarrierDummyTensorSize, 0, meta_,
            connection_ctx_, stream,
            [=](void*, size_t, size_t, const at::hip::HIPStreamMasqueradingAsCUDA&) {},
            [=](void*, size_t, size_t, const at::hip::HIPStreamMasqueradingAsCUDA&) {});
    }
}
...
c10::intrusive_ptr<c10d::Work> MooncakeBackend::scatter(
    ...
    if (isCpu_) {
        ...
        auto stream =
            at::hip::getCurrentHIPStreamMasqueradingAsCUDA(outputTensor.device().index());
        return worker_->putTaskCuda(
            c10d::OpType::SCATTER, tensorSize, root, meta_, connection_ctx_,
            stream,
            [=](void* dst, size_t pos, size_t realSize,
            ...
            [=](void* src, size_t pos, size_t realSize,
                const at::hip::HIPStreamMasqueradingAsCUDA& enq_stream) {
                hipMemcpyAsync((char*)outputTensor.data_ptr() + pos, src,
                                realSize, hipMemcpyDeviceToDevice, enq_stream);
            });
    }
}
    ...
    if (!has_hung_operation) {
        for (size_t i = 0; i < 2; i++) {
            ...
            if (isCpu_) {
                ...
                hipFree(send_buffer_[i]);
                hipFree(recv_buffer_[i]);
            }
        }
    }
}
...
int MooncakeBackend::getNumSyncedRanks() {
    ...
    if (!isCpu_) {
        auto stream =
            at::hip::getCurrentHIPStreamMasqueradingAsCUDA(tensors[0].device().index());
