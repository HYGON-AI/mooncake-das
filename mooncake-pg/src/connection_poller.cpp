...
// NVLink transport can only access hipMemCreate(FABRIC) memory
...
bool ConnectionContext::pollPeer(int pollingRank) {
    ...
    switch (peerState.state) {
        case PeerConnectionState::WAITING_STORE: {
            ...
            // A process may create several PG backends against the same peer
            // segment. Refresh the cached TE descriptor so buffers registered
            // by a later backend (for example, the CPU group used by EP) are
            // visible before selecting a transport for the warmup write.
