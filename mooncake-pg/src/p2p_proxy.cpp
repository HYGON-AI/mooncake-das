...
void SetCudaDeviceIfNeeded(bool is_cpu, int cuda_device_index,
    ...
    const hipError_t set_device_error = hipSetDevice(cuda_device_index);
    TORCH_CHECK(set_device_error == hipSuccess, context, ": ",
                hipGetErrorString(set_device_error));
}
...
P2PProxy::P2PProxy(TransferEngine* engine, const Options& options)
    ...
    if (!is_cpu_ && cuda_device_index_ < 0) {
        ...
        const hipError_t get_device_error = hipGetDevice(&current_device);
        TORCH_CHECK(get_device_error == hipSuccess,
                    "P2PProxy hipGetDevice failed: ",
                    hipGetErrorString(get_device_error));
...
void P2PProxy::AllocateResources() {
    ...
    if (is_cpu_) {
        ...
        SetCudaDeviceIfNeeded(
            ...
            "P2PProxy AllocateResources hipSetDevice failed");
        hipError_t err =
            hipMalloc(&resources_.send_buffer_, p2p_total_buffer_size);
        ...
        err = hipMalloc(&resources_.recv_buffer_, p2p_total_buffer_size);
        TORCH_CHECK(err == hipSuccess,
    ...
    if (!is_cpu_) {
        for (int peer_rank = 0; peer_rank < size_; ++peer_rank) {
            for (auto& copy_ready_event :
                ...
                const hipError_t create_error = hipEventCreateWithFlags(
                    &copy_ready_event, hipEventDisableTiming);
                TORCH_CHECK(create_error == hipSuccess,
                            "Failed to create pooled send copy-ready event: ",
                            hipGetErrorString(create_error));
            }
...
void P2PProxy::ReleaseResources() {
    ...
    SetCudaDeviceIfNeeded(is_cpu_, cuda_device_index_,
                          "P2PProxy ReleaseResources hipSetDevice failed");
    ...
    for (size_t i = 0; i < kMaxNumRanks; ++i) {
        ...
        for (auto& copy_ready_event : send_lane.copy_ready_events_) {
            ...
            const hipError_t destroy_error =
                hipEventDestroy(copy_ready_event);
            TORCH_CHECK(destroy_error == hipSuccess,
    ...
    if (resources_.send_buffer_ != nullptr) {
        ...
        if (is_cpu_) {
            ...
            hipFree(resources_.send_buffer_);
        }
    ...
    if (resources_.recv_buffer_ != nullptr) {
        ...
        if (is_cpu_) {
            ...
            hipFree(resources_.recv_buffer_);
        }
        resources_.recv_buffer_ = nullptr;
    }
}
...
bool P2PProxy::TryIssueSendTask(SendOpContext& op_ctx, uint32_t capacity) {
    ...
    if (is_cpu_) {
        ...
        hipError_t copy_error = hipMemcpyAsync(
            task.source_, tensor_ptr + task.chunk_offset_, task.chunk_bytes_,
            hipMemcpyDeviceToDevice, op_ctx.cuda_stream_);
        TORCH_CHECK(!copy_error, "P2P send hipMemcpyAsync failed: ",
                    hipGetErrorString(copy_error));
        const hipEvent_t pooled_copy_ready_event =
        ...
        copy_error =
            hipEventRecord(task.copy_ready_event_, op_ctx.cuda_stream_);
        if (copy_error != hipSuccess) {
            task.copy_ready_event_ = nullptr;
            TORCH_CHECK(false, "P2P send hipEventRecord failed: ",
                        hipGetErrorString(copy_error));
        }
    }
...
bool P2PProxy::StepSendDataCopy(SendTransferTask& task) {
    ...
    hipError_t query_error = hipSuccess;
    query_error = hipEventQuery(task.copy_ready_event_);

    if (query_error == hipSuccess) {
        task.copy_ready_event_ = nullptr;
        task.state_ = TransferState::kTransfer;
        return true;
    }
    if (query_error == hipErrorNotReady) {
        return false;
    }

    task.copy_ready_event_ = nullptr;
    TORCH_CHECK(false, "P2P send hipEventQuery failed: ",
                hipGetErrorString(query_error));
    return false;
}
...
bool P2PProxy::TryIssueRecvTask(RecvOpContext& op_ctx, uint32_t capacity) {
    ...
    if (is_cpu_) {
        ...
        hipError_t copy_error =
            hipMemcpyAsync(task.target_, task.source_, task.chunk_bytes_,
                            hipMemcpyDeviceToDevice, op_ctx.cuda_stream_);
        TORCH_CHECK(!copy_error, "P2P recv hipMemcpyAsync failed: ",
                    hipGetErrorString(copy_error));
        const hipEvent_t pooled_copy_ready_event =
            lane.copy_ready_events_[slot_index];
        TORCH_CHECK(pooled_copy_ready_event != nullptr,
        ...
        if (copy_error != hipSuccess) {
            task.copy_ready_event_ = nullptr;
            TORCH_CHECK(false, "P2P recv hipEventRecord failed: ",
                        hipGetErrorString(copy_error));
        }
    }
...
bool P2PProxy::StepRecvDataCopy(RecvTransferTask& task) {
    ...
    task.copy_ready_event_ = nullptr;
    TORCH_CHECK(false, "P2P recv hipEventQuery failed: ",
                hipGetErrorString(query_error));
    return false;
}
        ...
        if (IsRecvDataPathCompleted(op_ctx)) {
            if (!op_ctx.original_tensor_.is_contiguous()) {
                ...
                if (!is_cpu_) {
                    ...
                    TORCH_CHECK(sync_error == hipSuccess,
                                "P2P recv final copy hipDeviceSynchronize "
                                "failed: ",
...
void WorkerThreadLoop(bool is_cpu, int cuda_device_index,
    ...
    SetCudaDeviceIfNeeded(is_cpu, cuda_device_index,
                          "P2PDeviceWorker::WorkerThread hipSetDevice failed");
