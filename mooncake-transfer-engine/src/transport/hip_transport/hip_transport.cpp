...
#ifdef USE_HCU
#if USE_FAKE_HIP_RPC
// The base DTK runtime used by the non-HyLink variants does not provide
// hip_hsa_ext.h or hipFabricHandle_t. This type is only a compile-time token in
// fake-RPC builds: supportFabricMem() always returns false, so no object of this
// type is created, serialized, or consumed at runtime.
struct hipFabricHandle_t {};

// Stub the Hygon RPC entry points that are unavailable without HyLink.
...
static bool supportFabricMem() {
    ...
    std::call_once(warn_once, []() {
        LOG(WARNING) << "HipTransport: HCU fabric RPC is unavailable in this "
                        "variant; using IPC mode";
    });
    return false;
#endif
...
static std::string getCopyKernelPath() {
    ...
    std::call_once(flag, []() {
        ...
        Dl_info info{};
        if (dladdr(reinterpret_cast<void*>(&getCopyKernelPath), &info) != 0 &&
            info.dli_fname != nullptr) {
            std::filesystem::path module_path(info.dli_fname);
            std::filesystem::path packaged_kernel =
                module_path.parent_path() / "mc_copy_kernel.co";
            std::ifstream f(packaged_kernel);
            if (f.good()) {
                cached_path = packaged_kernel.string();
                LOG(INFO) << "HipTransport: Found packaged mc_copy_kernel.co: "
                          << cached_path;
                return;
            }
        }

        std::filesystem::path build_kernel =
            std::filesystem::path(__FILE__).parent_path() / "mc_copy_kernel.co";
        std::ifstream f(build_kernel);
        if (f.good()) {
            cached_path = build_kernel.string();
            LOG(INFO) << "HipTransport: Found build-tree mc_copy_kernel.co: "
                      << cached_path;
            return;
        }

        LOG(ERROR) << "HipTransport: mc_copy_kernel.co not found. "
                   << "Set MC_COPY_KERNEL_PATH or install the code object next "
                      "to the Mooncake shared libraries.";
    });
