...
Optional environment variables:
  HCU_TEST_TARGET_HOST      Target host (required)
  HCU_TEST_INITIATOR_HOST   Initiator host (required)
  ...
  HCU_TEST_INITIATOR_FILTER MC_TE_FILTERS on initiator (default: mlx5_0)
...
TARGET_HOST="${HCU_TEST_TARGET_HOST:?HCU_TEST_TARGET_HOST is required}"
INITIATOR_HOST="${HCU_TEST_INITIATOR_HOST:?HCU_TEST_INITIATOR_HOST is required}"
REMOTE_USER="${HCU_TEST_REMOTE_USER:-github}"
SSH_PORT="${HCU_TEST_SSH_PORT:-22}"
TARGET_FILTER="${HCU_TEST_TARGET_FILTER:-mlx5_6}"
INITIATOR_FILTER="${HCU_TEST_INITIATOR_FILTER:-mlx5_0}"
...
echo "Standard wheel cross-node RDMA read/write tests passed:"
...
if [ -n "${GITHUB_STEP_SUMMARY:-}" ]; then
    {
        ...
        echo "- Initiator: \`${INITIATOR_HOST}\` (\`MC_TE_FILTERS=${INITIATOR_FILTER}\`)"
