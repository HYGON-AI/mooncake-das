...
Run the Mooncake Store HTTP-metadata RDMA KV benchmark in temporary containers.
...
Optional environment variables:
  HCU_TEST_TARGET_HOST      Primary host (required)
  HCU_TEST_INITIATOR_HOST   Secondary host (required)
  ...
  HCU_STORE_SECONDARY_FILTER MC_TE_FILTERS on secondary (default: mlx5_0)
...
for command_name in ip cut awk docker flock getent grep python3 realpath scp sed ssh timeout; do
...
PRIMARY_HOST="${HCU_TEST_TARGET_HOST:?HCU_TEST_TARGET_HOST is required}"
SECONDARY_HOST="${HCU_TEST_INITIATOR_HOST:?HCU_TEST_INITIATOR_HOST is required}"
REMOTE_USER="${HCU_TEST_REMOTE_USER:-github}"
SSH_PORT="${HCU_TEST_SSH_PORT:-22}"
PRIMARY_FILTER="${HCU_STORE_PRIMARY_FILTER:-mlx5_6}"
SECONDARY_FILTER="${HCU_STORE_SECONDARY_FILTER:-mlx5_0}"
...
if ! ip -o -4 address show | awk '{print $4}' | cut -d/ -f1 | grep -Fxq "${PRIMARY_IP}"; then
    echo "ERROR: HCU_TEST_TARGET_HOST must resolve to this runner's local service address" >&2
    exit 1
fi
...
DOCKER_ENV_ARGS=(-e "HCU_TEST_DTK_PKG_URL=${HCU_TEST_DTK_PKG_URL}")
...
ssh "${SSH_OPTIONS[@]}" "${REMOTE}" bash -s -- \
    ...
    "${SECONDARY_CONTAINER}" "${HCU_TEST_IMAGE}" "${HCU_TEST_DTK_PKG_URL}" \
    "${PIP_INDEX_URL:-}" "${PIP_TRUSTED_HOST:-}" <<'REMOTE_CONTAINER' >/dev/null
set -Eeuo pipefail
remote_dir="$1"; wheel="$2"; script="$3"; container="$4"; image="$5"
dtk_url="$6"; pip_index="$7"; pip_trusted="$8"
env_args=(-e "HCU_TEST_DTK_PKG_URL=${dtk_url}")
...
set -Eeuo pipefail
timeout "$3" docker exec "$1" /bin/bash /work/run_hcu_store_kv_bench_ci.sh __container_prepare "/work/$2"
REMOTE_PREPARE
SECONDARY_PREP_PID=$!
...
echo "Mooncake Store cross-node RDMA write/read verification passed."
grep -E '=== |failed_requests|failed_kvs|misses|verify_failures|overall summary' \
    "${LOG_DIR}/store-kv-bench.log"
if [ -n "${GITHUB_STEP_SUMMARY:-}" ]; then
    {
        echo "### Mooncake Store HTTP-metadata RDMA KV benchmark"
        echo
        echo "- Primary: \`${PRIMARY_HOST}\`"
        echo "- Secondary: \`${SECONDARY_HOST}\`"
        echo "- write_verify: passed"
        echo "- Storage provider: \`${SECONDARY_HOST}\` only"
    } >>"${GITHUB_STEP_SUMMARY}"
fi
