...
if [ "$SHCA_BUILD" = "1" ]; then
    ...
    sed -i 's/description = "Python binding of a Mooncake library using pybind11"/description = "Python binding of a Mooncake library using pybind11 (TianLong SHCA version)"/' pyproject.toml
    sed -i 's/keywords = \["mooncake", "data transfer", "kv cache", "llm inference"\]/keywords = ["mooncake", "data transfer", "kv cache", "llm inference", "tianlong", "shca"]/' pyproject.toml
