...
usage() {
    cat <<'EOF'
Usage: ./scripts/variant_build.sh [options] <standard|rpc|shca>
...
setup_shca_env() {
    if [ -f /usr/include/infiniband/shca_17b_types.h ]; then
        echo "SHCA userspace headers are already installed."
        return 0
    fi
    if [ "${DRY_RUN}" -eq 1 ]; then
        echo "[dry-run] Would install SHCA userspace dependencies."
        return 0
    fi

    local setup_dir=/tmp/shca_setup
    mkdir -p "${setup_dir}"
    wget -q "${RESOURCE_SERVER_URL}/Jenkins/CompileDep/mooncake/mlxtoshca.sh" \
        -O "${setup_dir}/mlxtoshca.sh"
    wget -q "${RESOURCE_SERVER_URL}/Jenkins/CompileDep/mooncake/shca-tools_2.500.4.B068-Ubuntu22.04_amd64.deb" \
        -O "${setup_dir}/shca-tools_2.500.4.B068-Ubuntu22.04_amd64.deb"
    (cd "${setup_dir}" && bash ./mlxtoshca.sh)
    test -f /usr/include/infiniband/shca_17b_types.h
    ibv_devinfo
}
...
build_variant() {
    ...
    case "${variant}" in
        standard)
            ...
            cmake_args+=(-DUSE_HYGON=ON -DUSE_SHCA=OFF -DUSE_FAKE_HIP_RPC=ON)
            if [ "${DRY_RUN}" -eq 0 ]; then
                restore_pyproject
            fi
            ;;
        rpc)
            ...
            cmake_args+=(-DUSE_HYGON=ON -DUSE_SHCA=OFF -DUSE_FAKE_HIP_RPC=OFF)
            if [ "${DRY_RUN}" -eq 0 ]; then
                patch_pyproject \
    ...
    run_cmd cmake -S "${REPO_ROOT}" -B "${BUILD_DIR}" "${cmake_args[@]}" \
        ...
        -DENABLE_DEBUG_SYMBOLS=OFF -DUSE_HIP_DMABUF=OFF \
...
if [ "${BUILD_VARIANT}" = "shca" ]; then
    setup_shca_env
    export MOONCAKE_DEPS_SHCA=1
fi
