...
Build variants:
  standard  mooncake_transfer_engine       (normal NIC, no hylink; USE_HCU=ON, USE_FAKE_HIP_RPC=ON)
  rpc       mooncake_transfer_engine_rpc   (normal NIC, hylink enabled; USE_HCU=ON)
  shca      mooncake_transfer_engine_shca  (TianLong NIC, no hylink; USE_HCU=ON, USE_SHCA=ON, USE_FAKE_HIP_RPC=ON)
...
build_variant() {
    ...
    case "${variant}" in
        standard)
            ...
            cmake_args=(-DUSE_HCU=ON -DUSE_FAKE_HIP_RPC=ON)
            if [ "${DRY_RUN}" -eq 0 ]; then
                restore_pyproject
            fi
            ;;
        rpc)
            ...
            cmake_args=(-DUSE_HCU=ON -DUSE_FAKE_HIP_RPC=OFF)
            if [ "${DRY_RUN}" -eq 0 ]; then
                patch_pyproject \
        ...
        shca)
            ...
            cmake_args=(-DUSE_HCU=ON -DUSE_SHCA=ON -DUSE_FAKE_HIP_RPC=ON)
            if [ "${DRY_RUN}" -eq 0 ]; then
                patch_pyproject \
