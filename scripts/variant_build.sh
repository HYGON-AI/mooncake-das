...
Build variants:
  standard  mooncake_transfer_engine       (normal NIC, no hylink; USE_FAKE_HIP_RPC=ON)
  rpc       mooncake_transfer_engine_rpc   (normal NIC, hylink enabled)
  shca      mooncake_transfer_engine_shca  (TianLong NIC, no hylink; USE_SHCA=ON, USE_FAKE_HIP_RPC=ON)
...
build_variant() {
    ...
    case "${variant}" in
        standard)
            ...
            cmake_args=(-DUSE_HIP=ON -DUSE_FAKE_HIP_RPC=ON)
            if [ "${DRY_RUN}" -eq 0 ]; then
                restore_pyproject
            fi
            ;;
        ...
        shca)
            ...
            cmake_args=(-DUSE_HIP=ON -DUSE_SHCA=ON -DUSE_FAKE_HIP_RPC=ON)
            if [ "${DRY_RUN}" -eq 0 ]; then
                patch_pyproject \
