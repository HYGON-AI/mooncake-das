...
static __device__ void device_mutex_lock_system(uint32_t *mutex) {
    while (__hip_atomic_exchange(mutex, 1u, __ATOMIC_ACQUIRE,
                                 __HIP_MEMORY_SCOPE_SYSTEM) != 0);
}

static __device__ void device_mutex_unlock_system(uint32_t *mutex) {
    __hip_atomic_store(mutex, 0u, __ATOMIC_RELEASE, __HIP_MEMORY_SCOPE_SYSTEM);
}
...
static __device__ void __mlx5gda_device_post_send_db(
        struct mlx5gda_qp_devctx *ctx, uint32_t* gpu_hdp_reg) {
    // 0. flush hdp
    // WQE fields are written with ordinary stores. Publish them system-wide
    // before flushing HDP and notifying the NIC through DBR/Blue Flame.
    memory_fence();

    // Flush the GPU host data path before updating DBR and ringing Blue Flame.
    // System-scope fences order the WQE stores, HDP write, DBR, and doorbell.
    if (gpu_hdp_reg) {
        __hip_atomic_store(gpu_hdp_reg, (uint32_t)0x1, __ATOMIC_SEQ_CST,
...
dispatch(void* packed_recv_x, float* packed_recv_x_scales,
         ...
         void* hip_counter_buffer, void* hip_data_buffer,
         ...
         uint32_t* gpu_hdp_reg, int64_t timeout_ticks,
         int phases) {
    ...
    // DeepEP-style wave64 layout: every physical wave participates in packing,
    // then the block synchronizes before the final wave computes expert counts.
    {
        ...
        for (int token_idx = sm_id; token_idx < num_tokens; token_idx += num_sms) {
            ...
            for (int i = thread_id; i < hidden_bf16_int4; i += num_threads) {
                    ...
                    if (kUseFP8) {
                        ...
                        amax = quant_group_reduce_max(amax), scale = kFP8Amax / amax, scale_inv = amax * kFP8AmaxInv;
                        if (lane_id % (kNumPerChannels / kNumElemsPerRead) == 0)
                ...
                if (dst_rank != rank) {
                    ...
                    if (use_nvlink) {
                        ...
                        if (lane_id == 0) {
                            ...
                            //const uint16_t expect = ctx->wq_head - 1;
                            __mlx5gda_device_post_send_db(ctx, gpu_hdp_reg);
                            //__mlx5gda_device_poll_cq(ctx, expect);
    ...
    if (responsible_expert_idx < num_experts and sub_warp_id == 0 and lane_id == 0) {
        ...
        const auto wait_start = wall_clock64();
        int finish_counter;
        while ((finish_counter = ld_acquire_global(
                    atomic_finish_counter_per_expert +
                    responsible_expert_idx)) != FINISHED_SUM_TAG * 2) {
            if (timeout_ticks != -1 &&
                wall_clock64() - wait_start > timeout_ticks) {
                printf("Dispatch finish counter timeout: expert=%d value=%d "
                       "expected=%d sent=%d\n",
                       responsible_expert_idx, finish_counter,
                       FINISHED_SUM_TAG * 2, num_tokens_sent);
                trap();
            }
        }
        if (dst_rank != rank) {
            ...
            if (use_nvlink) {
                ...
                // The destination is a different GPU agent. Use a system-scope
                // release so the peer observes both the payload and the signal.
                st_release_sys_global(peer_signal_ptr, -num_tokens_sent - 1);
            } else {
    ...
    // Shared between the two physical waves in each expert group. The block
    // barrier below must be reached by every thread, including padding groups
    // in the final block when num_experts is not divisible by kNumWarpGroups.
...
void dispatch(void* packed_recv_x, float* packed_recv_x_scales,
              ...
              void* workspace, hipStream_t stream, uint32_t* gpu_hdp_reg,
...
LAUNCH_KERNEL(&cfg, dispatch_func, \
              ...
              hip_counter_buffer, hip_data_buffer, \
              ...
              num_topk, num_experts, rank, num_ranks, gpu_hdp_reg, timeout_ticks, phases); } break
...
combine(void* combined_x, int32_t* active_ranks,
    ...
    // All expert groups must finish their payload writes before any group
    // publishes its finishing flag. Padding groups also participate.
    EP_STATIC_ASSERT(kNumWarpsPerGroup > 1, "Requires more than one warp per group");
    __syncthreads();
        ...
        if (sub_warp_id == 1 and lane_id == 0) {
            ...
            if (dst_rank != rank) {
                ...
                if (use_nvlink) {
                    ...
                    // The destination is a different GPU agent. Use a system-scope
                    // release so the peer observes both the payload and the signal.
                    st_release_sys_global(peer_signal_ptr, 1);
                } else {
...
LAUNCH_KERNEL(&cfg, combine_func, \
              ...
              gpu_hdp_reg, timeout_ticks, phases, zero_copy); } break
