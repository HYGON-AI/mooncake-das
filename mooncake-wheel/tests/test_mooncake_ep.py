    ...
    for return_recv_hook in (False, True):
        for dispatch_use_fp8 in (False, True):
            ...
            for i in range(num_local_experts if do_check else 0):
                ...
                assert (recv_x[:, -128:] - recv_src_info.view(-1, 1) % num_tokens).abs().sum().item() == 0
                ...
                for j in range(num_ranks):
                    ...
                    if count:
                        assert (recv_segment[:, :-128] - (j - rank_offset)).abs().sum().item() == 0
...
def test_loop(local_rank: int, num_local_ranks: int):
    ...
    # Mock broken global rank 1 to test EP recovery. Using local_rank here would
    # incorrectly recreate one rank per node in a multi-node run.
    if rank != 1:
...
if __name__ == '__main__':
    ...
    num_processes = int(os.getenv("LOCAL_WORLD_SIZE", torch.cuda.device_count()))
