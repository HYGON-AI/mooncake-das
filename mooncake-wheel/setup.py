...
def get_base_version() -> str:
    ...
    match = re.search(
        r'^base-version\s*=\s*"([^"]+)"$', pyproject_content, re.MULTILINE
    )
    if match is None:
        raise RuntimeError(f"base-version is missing from {pyproject_path}")
...
def get_version_add() -> None:
    ...
    base_version_tuple = tuple(
        int(part) if part.isdigit() else part for part in base_version.split(".")
    )
    version = "das.opt1"
    ...
    if os.getenv("ROCM_PATH"):
        ...
        rocm_version = lines[0].strip().replace(".", "")
        version += ".dtk" + rocm_version

    sha = subprocess.check_output(
        ["git", "rev-parse", "--short=6", "HEAD"], cwd=deepep_root
    ).decode("ascii").strip()
    version += f".g{sha}"
...
try:
    ...
    __version_tuple__ = {base_version_tuple!r}
    __hcu_version__ = f'{base_version}+{version}'
